...
doSwap:
        ...
        li $t0, 0                       #initialize x to 0
       
        li $t1, 11                      #store value 11 for elems
        li $t2, 4                       #store offset of 4 bits for ints
...
loop:
        ...
        la $t7, myArray
        addu $t4, $t7, $t3          #get address of int x in myarray
        lw $t5, 0($t4)                  #store temp
        lw $t6, 4($t4)                  #store y

        sw $t6, 0($t4)                  #change int x to int y
        sw $t5, 4($t4)                  #change int y to temp

        addi $t0, $t0, 2                #increment x by 2
